...
int FileIO(char *fn1, struct IO io1, char *rw1)
{
  ...
  filename = fn1;
  io = io1;
  rw = rw1; //Save and check the calling
  if (rw[0] != 'r' && rw[0] != 'w')
    Error(525.); //parameters.

  init = trunc = count = 0; //Initialize work variables.

  fstr = extfstr(rw, rw[0] == 'w' ? "%g" : ""); //Extract any specification in the
  if (rw[0] == 'r')
  { //parameter and parse it if it
    if (fstr[0])
      mbparse(fstr); //represents a transformation of
    else
      fstr = 0; //input data.
  }
  ...
  for (i = 0; i < MDIM; i++)
    eol[i] = 0; //Clear the end-of-line array.
  ...
  {
    c = abs(io.mm[i - 2]);
    d = io.mm[i - 1]; //width of each dimension for use
    if (c >= 'A' && c <= 'Z')
      c += 'a' - 'A'; //in the nested loops.
    clim[c - 'a'] = d;
    cwth[c - 'a'] = w;
  }
...
{
  dec w;
  int c, i, i0, i1, di, iw, sep[2];

  if (o >= z && io.sm[0])
    Error(920.); //Check the range of the offset.

  if (io.sm[0])
    c = -io.sm[o++]; //Retrieve the present index label
  else
  {
    c = -io.mm[o++];
    o++;
  } //and make sure it is within range.
  if (c < 0 || c - 'a' >= MLAB)
    Error(515.2);
  ...
  if (c >= 'A' && c <= 'Z') //If the label is capitalized, mark
  {
    c += 'a' - 'A';
    eol[level] = 1;
  } //a new-line and make sure the
  if (clim[c - 'a'] == 0)
    Error(515.3); //the label was defined.

  i0 = 0;
  i1 = clim[c - 'a'] - 1;
  di = 1;       //Develop the indexing range and
  if (io.sm[0]) //increments, using default values
  {
    if (o < z && io.sm[o] >= 0)
      i0 = io.sm[o++]; //if 'mm' is being used or if they
    if (o < z && io.sm[o] >= 0)
      i1 = io.sm[o++]; //are not specified in 'sm'.
    if (o < z && io.sm[o] > 0)
      di = io.sm[o++];
  }
  iw = di * cwth[c - 'a'];
  ...
  if (i0 >= clim[c - 'a'])
    Error(517.1); //Make sure the index values are
  if (i1 >= clim[c - 'a'])
    Error(517.2); //within range and the range is
  if ((abs(i1 - i0) + 1) % abs(di))
    Error(518.0); //divisible by the increment.
  ...
  if (level > 0) //If this is not the lowest level,
  {
    for (i = i0; di > 0 ? i <= i1 : i >= i1; i += di) //loop through this level and all
    {
      ...
      nest(o, base + iw * i, level - 1);
    }                               //codes at the end of specified
  ...
  if (rw[0] == 'r') //At the lowest level for reading,
  {
    for (i = i0; di > 0 ? i <= i1 : i >= i1; i += di) //read all data at this level into
    {
      ...
      if (fstr)
        w = w * xm + xb; //optionally truncating to an
      if (trunc)
        w = (int)w; //integer.
      io.data[base + i * iw] = w;
    }
  ...
  if (rw[1])
    sep[0] = rw[1]; //retrieve the field separator.
...
static mbparse(char *p)
{
  ...
  i = *p++;
  trunc = i == 'n'; //Check for integer truncation.
  if (i != 'n' && i != 'x')
    Error(520.); //Check the start of the string.
  xm = 1;
  xb = 0; //Assume an identity transformation.

  switch (*p)
  {
  case 0:
    return 0; //Tendai changed return to return 0; to avoid warnings                          //No transformation specified.
...
static int CentinelWrite(int c, int i0, int i1, int di, int base, int iw)
{
  ...
  if (init == 0) //If this is the first call, write
  {
    ...
    if (c < 'a')
      c += 'a' - 'A'; //Make the current label lower-case.
  ...
  {
    if (io.sm[0])
      d = -io.sm[i]; //index values of the higher
    else
      d = -io.mm[i]; //dimensions.
    if (d <= 0)
      continue;
    if (d < 'a')
      d += 'a' - 'A';
...
static int StoreColumns()
{
  ...
  for (cmax = 0; cmax < MCOL; cmax++) //Advance to the beginning of the
  {
    c = advance(0);
    if (c != '|')
      cerr(840.1); //next column.

    byp(0);
    c = advance(0); //Extract the label and store it
    if (c < 'a' || c > 'z')
      cerr(524.1); //for this column.
    clabel[cmax] = c;
    ...
    if (imax == 1 && n < 0 && c == 'z') //if any, and store it too, with
    {
      n = 0;
      clim[c - 'a'] = cwth[c - 'a'] = 1;
    }                 //special handling for variable 'z'
    cindex[cmax] = n; //in one-dimensional arrays.

    byp(0);
    c = advance(0); //If another field follows
    if (c == '|')
    {
      retract(c);
      continue;
    ...
    if (c != '\n' && c != '\r')
      cerr(523.);                   //If the line has ended normally,
    for (f = i = 0; i <= cmax; i++) //scan to make sure any index
    {
      if (cindex >= 0)
        f += 1; //ranges precede all data values,
      else if (f)
        cerr(530.);
    } //and that data values exist.
    if (f == 0)
      cerr(531.);
...
static int StoreData()
{
  ...
  for (cnt = k = i = 0; i <= cmax; i++) //Advance to the beginning of the
  {
    c = advance(0);
    if (c != '|')
      cerr(840.2); //next column and determine whether
    byp(0);
    use = clim[clabel[i] - 'a'] > 0; //it is to be used in the array.

    if (use && cindex[i] < 0) //If this is a set of indexes only,
    {
      if (k >= MDIM)
        Error(526.); //extract all ranges.
    ...
    else if (use) //If it is a number to be spread
    {
      field(f, FLTH);
      if (f[0] == ' ')
        break;            //about the array, extract its
      x = strtod(f, &nf); //value and rescale it as needed,
      if (fstr)
        x = x * xm + xb; //optionally truncating to an
      if (trunc)
        x = (int)x; //integer.

      for (j = nf - f; f[j] == ' '; j++)
        ; //Make sure the field does not
      if (f[j])
        cerr(533.1); //carry spurious characters.

      w = cwth[clabel[i] - 'a']; //Determine the base offset in the
      w *= cindex[i];            //array and distribute the element
      cnt += StoreElement(x, w);
    } //through the array.
    ...
    if (c == '|')
      continue; //immediately, repeat.

    if (c != '\n' && c != '\r')
      cerr(533.2); //If the line has ended normally,
    BypLine();
    return cnt;
  } //advance to the next and return.
...
static ranges(int d, int s[], int k)
{
  ...
  if (d < 'a' || d > 'z')
    cerr(524.2); //Check the label for validity.

  for (i = 0; i < k; i += 2) //Retrieve the first value of a
  {
    r0 = posint(0);
    if (r0 < 0)
      cerr(534.1);        //pair and save it as if it is a
    ...
    if (c == '~')   //retrieve the second value of the
    {
      r1 = posint(0);
      if (r1 < 0)
        cerr(534.2); //pair and store a range.
      if (r0 <= r1)
        s[i + 1] = r1;
      else
        s[i] = r1;
    ...
    if (s[i + 1] > m)      //values that are outside the
    {
      if (!c1++)
        cerr(387.);
      s[i + 1] = m;
    } //array, giving a warning message
    if (s[i] > m)
      i -= 2; //just once.

    if (c == ',')
      continue; //Continue if another range follows
    if (c == ' ')
    {
      byp(0);
      c = advance(0);
    } //or return if it does not.
    if (EOC)
    {
      s[i + 2] = -1;
      retract(c);
...
static int StoreElement(dec x, int k0)
{
  ...
  for (cnt = 0;;)
  {
    ...
    for (i = m; i >= 0; i--) //Advance to the next index,
    {
      ...
      if (V[i] <= drange[i][I[i] + 1])
        break; //lower level wraps around.
      I[i] += 2;
      V[i] = drange[i][I[i]];
      if (V[i] >= 0)
        break;
    ...
    if (i < 0)
      return cnt;
  } //Return when all values have been
} //stored.
...
static field(char *s, int k)
{
  ...
  for (i = 0; i < k - 1; i++) //Retreive the next character.
  {
    ...
    if (EOC) //If the field is exhasuted, close
    {
      ...
      s[i] = 0;
      return 0;
    } //Tendai changed return to return 0; to avoid warnings        //the string and return.

    s[i] = c;
  } //Store the character and repeat.
...
static int posint(int rtneof)
{
  ...
  byp(0);
  n = advance(0);         //If the file is not positioned
  ...
  while (1) //If it is, read any remaining
  {
    d = advance(0);
    if (d < '0' || d > '9')
      break; //digits to complete the integer.
    n = n * 10 + (d - '0');
  }

  retract(d);
  byp(0);   //Bypass any trailing blanks and
...
static int BypLine()
{
  ...
  while (1) //Advance to the next characters
  {
    c = getc(fp);
    if (c == EOC)
      return 1; //and return at end of file or end
    if (c == '\n')
      return 0; //of line.
    ...
    if (c == '\r') //Keep skipping line returns until
    {
      c = getc(fp);
      if (c == EOC)
        return 1; //end of file or end of line.
      if (c == '\r')
        continue;
      if (c == '\n')
        return 0;
