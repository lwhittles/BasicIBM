...
{ int i; char *np;
  ...
  switch(*p)
  { case 0: return 0;  //Tendai changed return to return 0; to avoid warnings                          //No transformation specified.
    ...
    casemd:                                  //Continuing multiplication and
      ...
      { default:  Error(522.1);
        case 0:   return 0;  //Tendai changed return to return 0; to avoid warnings
        case '+': case '-': break; }

    case '+': case '-':                      //Addition or subtraction.
      xb = strtod(p, &np);
      if(*np==0) return 0;  //Tendai changed return to return 0; to avoid warnings
...
{ int c, i, m, r0, r1;
  ...
  { r0 = posint(0); if(r0<0) cerr(534.1);    //pair and save it as if it is a
    ...
    if(EOC) { s[i+2] = -1; retract(c); return 0;}  //Tendai changed return to return 0; to avoid warnings
...
{ int i, c;
  ...
  { c = advance(0);
    ...
    { retract(c); s[i] = 0; return 0;}  //Tendai changed return to return 0; to avoid warnings        //the string and return.
