...
ENTRY: 'tab' contains a table of times of future events.
       ...
       'MaxE' highest t index
       'T0' earliest simulation time
...
/*Old way of calculating earliest

int Earliest(dec tab[], int subset[])
{ int i, m; dec x, w;

  for(x=100000000,m=i=0; subset[i]>=0; i++) //Tendai: Ignore event s not yet timed.
    { w = tab[subset[i]]; if(x>w && w>=T0) { x = w; m = i; } }
